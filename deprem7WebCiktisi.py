...
with open('rapor.html', 'w') as html_file:
    ...
    html_file.write('<tr><th class="lead">Şehir</th><th class="lead">Bina Sayısı</th><th class="lead">Etkilenen Bina</th><th class="lead">Yıkılan Bina</th><th class="lead">Hasarlı Bina</th><th class="lead">Deprem Şiddeti</th><th class="lead">Deprem Süresi</th><th class="lead">Deprem Derinligi</th><th class="lead">Deprem Uzaklığı</th></tr>\n')
    etkilenenBinaSayısı = 0
    hasarliBinaSayısı = 0
    yikilanBinaSayisi = 0
    binaSayisi = 0
    ...
    for rapor in raporListesi:
        ...
        html_file.write(f'<td>{int(rapor.yikilanBinaSayisi*0.20)}</td>')
        html_file.write(f'<td>{int(rapor.yikilanBinaSayisi*0.80)}</td>')
        ...
        etkilenenBinaSayısı += rapor.yikilanBinaSayisi 
        yikilanBinaSayisi += int(rapor.yikilanBinaSayisi*0.20)
        hasarliBinaSayısı += int(rapor.yikilanBinaSayisi*0.80)


    html_file.write(f'<td></td><td>{binaSayisi}</td><td>{etkilenenBinaSayısı}</td><td>{yikilanBinaSayisi}</td><td>{hasarliBinaSayısı}</td><td></td><td></td><td></td><td></td></table>\n')
